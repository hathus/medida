...
new #[Layout('layouts.guest')] class extends Component
{
    ...
    public function login(): void
    {
        ...
        $this->redirectIntended(default: route('recetas', absolute: false), navigate: true);
    }
}; ?>
...
<div>
    ...
    <form wire:submit="login" novalidate>
        <!-- Email Address -->
        <div>
            <x-input-label for="email" :value="__('login.form.email')" />
        ...
        <!-- Password -->
        <div class="mt-4">
            <x-input-label for="password" :value="__('login.form.password')" />
        ...
        <div class="block mt-4">
            <label for="remember" class="inline-flex items-center">
                ...
                <span class="text-sm text-gray-600 ms-2 dark:text-gray-400">{{ __('login.form.remember') }}</span>
            </label>
        </div>
        ...
        <div class="flex items-center justify-end mt-4">
            @if (Route::has('password.request'))
                <a class="text-sm text-gray-600 underline rounded-md dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800" href="{{ route('password.request') }}" wire:navigate>
                    {{ __('login.form.forgot') }}
                </a>
            @endif

            <x-primary-button class="ms-3">
                {{ __('login.form.login_btn') }}
            </x-primary-button>
        </div>
    </form>
</div>
