    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-gray-100 dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg border border-lime-300">
            @forelse ($expedientes as $expediente)
                <div
                    class="p-6 text-gray-900 dark:text-slate-200 md:flex md:justify-between md:items-center border border-lime-300">
                    <div class="space-y-3 rounded-lg">
                        <a href="{{ route('mostrar-expediente', $expediente->id) }}" class="font-bold">
                            {{ $expediente->name }}
                        </a>
                        <p>Última Consulta:
                            {{ $carbon::parse($expediente->consultas->last()->created_at)->translatedFormat('l j \de F \de Y') }}
                        </p>
                        ...
                        <p>Glucosa:
                            <span
                                class="text-sm font-bold flex justify-start items-center {{ $expediente->consultas->first()->glucose <= 120 ? 'text-lime-600' : 'text-red-600' }}">
                                @if ($expediente->consultas->first()->glucose <= 120)
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-lime-400">
                                    ...
                                    {{ $expediente->consultas->first()->glucose }} -> Glucosa Controlada
                                @else
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    ...
                                    {{ $expediente->consultas->first()->glucose }} -> Glucosa Descontrolada
                                @endif
                            </span>
                        </p>
                        <p>
                            IMC:
                            @if ($expediente->consultas->first()->imc < 18.5)
                                <span class="flex text-sm font-bold text-blue-600 justify-start items-center">
                                    ...
                                    {{ round($expediente->consultas->first()->imc, 2) }}
                                    -> Bajo Peso
                                </span>
                            @elseif ($expediente->consultas->first()->imc >= 18.5 && $expediente->consultas->first()->imc <= 24.9)
                            ...
                            @elseif ($expediente->consultas->first()->imc >= 25 && $expediente->consultas->first()->imc <= 29.9)
                    ...
                    <div class="flex flex-col md:flex-row items-stretch gap-3 mt-5 md:mt-0">
                        ...
                        <a href="{{ route('mostrar-expediente', $expediente->id) }}"
                            class="py-2 px-4 rounded border border-green-700 dark:text-slate-100 hover:bg-green-700  text-xs font-bold uppercase flex items-center justify-center hover:text-white">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        ...
                        <a href="{{ route('editar-expediente', $expediente->id) }}"
